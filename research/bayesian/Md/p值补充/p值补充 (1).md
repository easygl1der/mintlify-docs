...
$\succ$ 统计量为 $T(y) = \frac{\bar{y} - \theta_0}{1 / \sqrt{n}}$
...
反之，若 $P(\overline{y}^{\mathrm{rep}} > \overline{y})$ 接近 0 或接近 1；则认为 $\mathrm{H}_{0}$ 不成立。
...
若此概率接近1, 则拒绝 $\mathrm{H}_{0}$ 。进一步, $P\{N(0,1) > T(y)\} \approx 1$ 等价于 $P\{N(0,1) < T(y)\} \approx 0$ 。由于 $\bar{y}^{\text {rep}} < \bar{y}$ 为小概率事件, 故认为 $\theta_{0} > \theta$ , 即 $\mathrm{H}_{1}: \theta < \theta_{0}$ 成立。
