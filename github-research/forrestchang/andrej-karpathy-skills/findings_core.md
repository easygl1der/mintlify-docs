...
| **创建时间** | 2023-05-10 |
| **最后推送** | 2026-04-01 |
| **主要Topics** | machine-learning, deep-learning, pytorch, tutorials, educational |

## 项目简介

andrej-karpathy-skills 是一个专注于机器学习与深度学习技能学习的代码仓库，由社区成员 @forrestchang 创建并维护，汇集了 Andrej Karpathy 的教学内容和代码实现，旨在帮助开发者学习和掌握现代深度学习的核心技能。
...
```python
# 场景1：运行Jupyter Notebook教程
# 假设已经克隆仓库并安装了依赖
jupyter notebook notebooks/01-neural-networks-basics/building-neural-networks.ipynb

# 场景2：直接运行Python脚本
python scripts/model_training.py --data-path ./data/samples/ --epochs 10

# 场景3：使用预处理脚本准备数据
python scripts/data_preprocessing.py --input ./raw_data/ --output ./processed_data/

# 场景4：模型评估和测试
python scripts/evaluation.py --model ./models/trained_model.pt --test-data ./data/test/
```
...
**Python** — 基于项目定位和 Andrej Karpathy 的教学风格，该项目的主要编程语言是 Python：

- **机器学习领域标准**：Python 是机器学习和深度学习领域事实上的标准语言
- **丰富的库生态**：拥有 NumPy、Pandas、Matplotlib、TensorFlow、PyTorch 等成熟库
- **教学友好**：语法简洁易学，适合作为编程入门和教学语言
- **社区支持**：机器学习教育资源中 Python 文档和示例最为丰富

### 预计 Python 版本

Python 3.6+ / 3.8+（考虑到现代深度学习框架的版本要求）
